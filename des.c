...
#if !defined(GARY_LI_)
/*
 * Wrapper for 3DES-ECB block encryption/decryption
 */
int Des2Ecb(const unsigned char key[16], const unsigned char data_in[16], unsigned char data_out[16], const unsigned char datalen8, const unsigned char isEncrypt)
{
	des3_context ctx3;
	if (isEncrypt == DES_ENCRYPT)
	{
		...
		if (datalen8)
		{
			des3_crypt_ecb( &ctx3, data_in, data_out );
		}
		else
		{
			des3_crypt_ecb( &ctx3, data_in, data_out );
			des3_crypt_ecb( &ctx3, data_in+8, data_out+8 );
		}
	}
	else
	{
